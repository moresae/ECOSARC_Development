...
DATA_NAME = 'n35_bmode_data.npy'
LABELS_NAME = 'n35_bmode_labels.npy'
MASK_NAME = 'n35_bmode_mask.npy'

prepare_train_test_split = False
test_set_list = [[12, 13, 16, 19, 20, 27, 33], [2, 3, 9, 24, 28, 32], [6, 22, 23, 29, 30, 35], [4, 8, 15, 17, 26, 34], [1, 7, 10, 18, 25, 31]]
cross_validation_indices = [4]
if prepare_train_test_split == True:
    id_test_set = 0
    for kfold in cross_validation_indices:
        print('kfold' + str(kfold))
        test_set = test_set_list[kfold]
        ...
        np.save(path_to_raw + 'train' + str(kfold) + '_' + DATA_NAME, bmode_data_train)
        np.save(path_to_raw + 'test' + str(kfold) + '_' + DATA_NAME, bmode_data_test)
        np.save(path_to_raw + 'train' + str(kfold) + '_' + LABELS_NAME, bmode_labels_train)
        np.save(path_to_raw + 'test' + str(kfold) + '_' + LABELS_NAME, bmode_labels_test)
        np.save(path_to_raw + 'train' + str(kfold) + '_' + MASK_NAME, bmode_mask_train)
        np.save(path_to_raw + 'test' + str(kfold) + '_' + MASK_NAME, bmode_mask_test)
...
if load_existing_dataset:
    ...
    if (skipRFvsBmodeConversion == False):
        #Variables
        whichdata = ['trainRF_array', 'testRF_array']
        #list_labels_modify_functions = [ ["_psden", stages_rf2D_ps_denoising]]
        list_labels_modify_functions = [  ["_iq", stages_rf2D_rftobmode_iq], ["_env", stages_rf2D_rftobmode_env], ["_log",stages_rf2D_rftobmode_log], ["_den",stages_rf2D_rftobmode_den], ["_psden", stages_rf2D_ps_denoising], ["_ps", stages_rf2D_ps]]
        #Data evaluation loop
        for xdata in whichdata:
            #Open data
            RF_array = np.load( (path_to_raw + xdata + '.npy')) # same size as trainRF_array, contains segmentation masks    
            ...
            for xfun in list_labels_modify_functions:
                ...
                if custom_save_label[1:3] == 'ps':
                    RF_array_transform = np.zeros([Nframes, 111, 192, 33])
                else:
                ...
                #ax.set_aspect('auto')        
                #plt.show()
                np.save(inputnn + xdata + custom_save_label + '.npy', RF_array_transform)
            del RF_array
        ...
        for kfold in cross_validation_folds:   
            ...
            for xdata in whichdata:
                ...
                for index in range(Nframes):
                    ...
                    mask_patch, coverage[index], where_patches = mask_patch_extraction(RF_array[index,:,:], largest_connected_region, Naxial, Nlines, Npatches) # mask_patch (393, 27, 15) <> +1 in axial coordinate to have odd number
                    if coverage[index] > 0:
                ...
                np.save(inputnn + xdata  + str(kfold)+ 'B_patches_' +   'array.npy', patches2D[0:idwrite,:,:,:])
                np.save(inputnn + xdata  + str(kfold)+ 'B_patches_' +  '_labels2D.npy', labels2D[0:idwrite,:])
                np.save(inputnn + xdata  + str(kfold)+ 'B_patches_' +  '_labels1D.npy', labels1D[0:idwrite1D,:])
                np.save(inputnn + xdata  + str(kfold)+ 'B_patches_' +  '_where_patches_receipt.npy', where_patches_receipt[0:idwrite,:])
    ...
    if (skipGenerate2DPatchesFromRFvsBmode == False):
        ...
        #Variables
        whichdata = ['train', 'test']
        list_labels_modify = ["_ps", "_psden",   "_iq", "_env", "_log", "_den"]
        #Data evaluation loop
        for xdata in whichdata:
            #Open data and patch extraction receipt
            where_patches_receipt = np.load((inputnn + xdata + 'RF_patches__where_patches_receipt.npy'))
            ...
            for xmodifier in list_labels_modify:
                if (xmodifier[1:3] == 'ps'):
                    ...
                    patches2D = np.zeros([Nframes*Npatches, Nlines, Naxial, 1]) # Big array where data is stored
                # Open data
                data = np.load( (inputnn + xdata + 'RF_array' + xmodifier + '.npy')) # same size as trainRF_array, contains segmentation masks    
                idwrite = 0   # For generating patches2d and labels2d
                for indexF in range(Nframes):
                    ...
                    if xmodifier[1:3] == 'ps': # For _ps and _psden
                        ...
                        where_coords_iq2[:,0] = np.floor(np.floor(where_coords[:, 0]/2) + 2928/2)
                ...
                #plt.show()
                print('Storing 2D patch data...')
                np.save(inputnn + xdata + 'RF_patches_' + 'array' + xmodifier + '.npy', patches2D[0:idwrite,:,:,:])
    SystemExit()
...
outputnnmodel = "../outputnnmodel" + MODIFIER + "/"
outputnnresults = "../outputnnresults" + MODIFIER + "/"
numClasses = 4
...
list_labels_training = [
                      ...
                      #  ["array_den", "_labels1D", stages_reshape2D_1Dpatch, model_1D_210418_08b, "Run210421_model_1D_210411_08b", dict_train_settings1D],
                        ["array", "_labels2D", stages_dont_do_anything, PHILIPS_N35_bmode_model_2D_210411_06b_4class, "Run210424_PHILIPS_N35_bmode_model_2D_210411_06b_0p01LR_30batch" + label_multiclass, dict_train_settings2Dbis],
                    #    ["array", "_labels2D", stages_dont_do_anything, PHILIPS_N35_bmode_model_2D_210411_06b_4class, "Run210430_PHILIPS_N35_bmode_model_2D_210411_06b_0p01LR_30batch" + label_multiclass, dict_train_settings2Dbis],
                    # ["array", "_labels2D", stages_dont_do_anything, PHILIPS_N35_bmode_model_2D_210430_4class, "Run210430_PHILIPS_N35_bmode_model_2D_210430_4class" + label_multiclass, dict_train_settings2Dbis],
                    ["array", "_labels2D", stages_dont_do_anything, PHILIPS_N35_bmode_model_2D_210430_4class, "Run210526_PHILIPS_N35_bmode_model_2D_210430_4class" + label_multiclass, dict_train_settings2Dbis],
                    ["array", "_labels2D", stages_dont_do_anything, PHILIPS_N35_bmode_model_2D_210411_06b_4class, "Run210526b_PHILIPS_N35_bmode_model_2D_210430_4class" + label_multiclass, dict_train_settings2Dbis],
                      # ["array_iq", "_labels2D", stages_dont_do_anything, model_2D_210411_06, "Run210421_model_2D_210411_06", dict_train_settings2D],
                      #  ["array_env", "_labels2D", stages_dont_do_anything, model_2D_210411_06b_v02, "Run210421_model_2D_210411_06bis_lowerrate", dict_train_settings2D],
                      #  ["array_log", "_labels2D", stages_dont_do_anything, model_2D_210411_06b_v02, "Run210421_model_2D_210411_06bis_lowerrate", dict_train_settings2D],
                      #  ["array_den", "_labels2D", stages_dont_do_anything, model_2D_210411_06b, "Run210421_model_2D_210411_06bis", dict_train_settings2D],
                      #  ["array_ps", "_labels2D", stages_reshape3D_4Dpatch, model_3D_210411_07_optim, "Run210421_model_3D_210411_07bis", dict_train_settings3D],
                     #    ["array_ps", "_labels2D", stages_reshape3D_2Dpatch, model_2D_210411_06, "Run210421_model_2D_210411_06", dict_train_settings2D],
                     #   ["array_psden", "_labels2D", stages_reshape3D_4Dpatch, model_3D_210411_07_optim, "Run210421_model_3D_210411_07bis", dict_train_settings3D],
                      # ["array_psden", "_labels2D", stages_reshape3D_2Dpatch, model_2D_210411_06,  "Run210421_model_2D_210411_06", dict_train_settings2D],                        
                        ]
...
if skipTraining == False:
    ...
    for x in list_labels_training:
        ...
        for kfold in cross_validation_folds:        #Data evaluation loop
            ...
            if (kfold ==-14):
                destination_model =  outputnnmodel + model_label + '__' + dataset_name 
            ...
            train_val_RAM = True
            train_data = np.load(inputnn + "train" + str(kfold) + "B_patches_"  + dataset_name + '.npy') # Already prepared for training
            train_data = modify_fun(train_data)  # Perform reshaping if necessary - check memory concerns
            val_data = np.load(inputnn + "test" + str(kfold) + "B_patches_" +  dataset_name + '.npy')
            ...
            # Get labels
            labels_train = np.load(inputnn + "train" + str(kfold) + "B_patches_" + labels_name + '.npy')
            labels_val = np.load(inputnn + "test" + str(kfold) + "B_patches_" + labels_name + '.npy')
            ...
            # Second run with old settings
            my_optimizer = tf.keras.optimizers.RMSprop(learning_rate=0.001, rho=0.9)
    ...
    ########################################## TRAINING LOOP ######################################
            ...
            MODEL_RETRIEVE_MODE = 1

            if MODEL_RETRIEVE_MODE == 0:
            #------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
            # if data has been loaded into RAM
                ...
                history = model.fit(
                                    ...
                                    shuffle=True,
                                    #workers=1,
                                    #use_multiprocessing=False                                    class_weight = weights
                                )
                checkpoint_filepath =   outputnnmodel + '/model_weights_checkpoint.hdf5'
                ...
                model.save( (outputnnmodel +model_label  + '__' + dataset_name  + "_kfold" + str(kfold)+ '/model') )
                gc.collect()
                ...
                try:
                    model = tf.keras.models.load_model(outputnnmodel+ model_label + '__' + dataset_name + "_kfold" + str(kfold) + '/model')
                except:
    ...
    ##### DATA ANALYSIS AND EVALUATION LOOP ##################################################################################################
            ...
            if do_pred_train:
                ...
                if MODEL_RETRIEVE_MODE == 0:
                    ...
                    train_real_labels_i, train_test_maxfrac_i, train_test_labels_i, train_metrics_i = results_instance_retrieve_scores( pred_train , labels_train_1hot , destination_results, '_train_')
                    np.save(destination_results + "/train_test_labels_frac_retrieve.npy", np.array(train_test_maxfrac_i))
                    
                    print('\n\\\\\\\\\\ CASE LEVEL /////')
                    train_real_labels_c, train_test_case_df, train_test_labels_c, train_metrics_c, train_labels_frac_c = results_case_retrieve_scores( pred_train , labels_train,  destination_results, '_train_', threshold  )
                    np.save(destination_results + "/train_real_labels_c_.npy", np.array(train_real_labels_c))
                    np.save(destination_results + "/train_test_labels_c_.npy", np.array(train_test_labels_c))
                    np.save(destination_results + "/train_test_labels_frac_c_.npy", np.asarray(train_labels_frac_c))
            ...
            if do_pred_val:
                ...
                if MODEL_RETRIEVE_MODE == 0:
                    ...
                    print('\n\\\\\\\\\\ CASE LEVEL /////')
                    val_real_labels_i, val_test_maxfrac_i, val_test_labels_i, val_metrics_i = results_instance_retrieve_scores(  pred_val ,labels_val_1hot , destination_results, '_val_')
                    np.save(destination_results + "/val_test_labels_frac_retrieve.npy", np.array(val_test_maxfrac_i))
                
                    val_real_labels_c, val_test_case_df, val_test_labels_c, val_metrics_c, val_labels_frac_c = results_case_retrieve_scores( pred_val , labels_val,  destination_results, '_val_', threshold )
                    np.save(destination_results + "/val_real_labels_c_.npy", np.array(val_real_labels_c))
                    np.save(destination_results + "/val_test_labels_c_.npy", np.array(val_test_labels_c))
                    np.save(destination_results + "/val_test_labels_frac_c_.npy", np.asarray(val_labels_frac_c))
