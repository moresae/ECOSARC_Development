...
#1
#------------------------------------------------------------
# Ignacio Oyarzabal, 2022.

exec(open("/home/estudiante/rfvsbmode_Ignacio/rfvsbmode_Ignacio/0_settings_preprocessing_rfvsbmode_OMAR.py").read())
#exec(open("0_settings_sergiorfvsbmodeliver_OMAR.py").read())
exec(open("/home/estudiante/rfvsbmode_Ignacio/rfvsbmode_Ignacio/0_settings_sergiorfvsbmodeliver_OMAR.py").read())
...
path_to_raw = '/home/estudiante/rfvsbmode_Ignacio/rfvsbmode_Ignacio/datosEcogeneicidad/' #datosGrasa3vsResto
dir_files_drive = '/bifrost2/home/ssanabria/ECOFRAIL/ALBACETE_ECOSARC_RETROSPECTIVO/'
imagenesValidas = []
LABELS_NAME = 'LabelsEcofrail.npy'
'''
for file in glob.iglob(dir_files_drive + 'Imagenes_Proyecto/ImagenesValidas/*', recursive=True):
  imagenesValidas.append(file)

bmode_labels = np.load(dir_files_drive + LABELS_NAME , allow_pickle=True)
#recoger los indeces de las imagenes que estan vacias y borrarlos
indice = []
for imagenes in imagenesValidas:
    i=0
    for imagenesBien in bmode_labels[:,7]:
        if imagenesBien[105:-4] in imagenes:
            print('Imagen numero: ',i)
            indice.append(i)
        i=i+1        
bmode_labels = pd.DataFrame(bmode_labels)
bmode_labels.drop(indice, axis=0)
bmode_labels = np.array(bmode_labels)

### Prepare train and test sets


bmode_data = np.load(dir_files_drive + DATA_NAME)    
bmode_labels = np.load(dir_files_drive + LABELS_NAME , allow_pickle=True)
bmode_mask = np.load(dir_files_drive + MASK_NAME)

for i in range(0,len(bmode_data)):
    if bmode_mask[i:,:,:].sum() ==0:
        print('Esta Vacio')
    if bmode_data[i:,:,:].sum() ==0:
        print('Esta Vacio Datos')   ''' 
...
if prepare_train_test_split == True:
    ...
    #Borrar las mascaras vacias y labels vacias
    labelsVacias = (bmode_labels[:,0][bmode_labels[:,2]==-1]).astype(int)
    bmode_mask = np.delete(bmode_mask,labelsVacias,axis=0)
    bmode_data = np.delete(bmode_data,labelsVacias,axis=0)
    #cuadradar mascaras vacias, datos y labels por indice
    copiaLabels3 = bmode_labels[:,2]
    copiaLabels3 = np.delete(copiaLabels3,labelsVacias)
    copiaLabels0 = bmode_labels[:,0]
    copiaLabels0 = np.delete(copiaLabels0,labelsVacias)
    copiaLabels1 = bmode_labels[:,1]
    copiaLabels1 = np.delete(copiaLabels1,labelsVacias)
    for i in range (0,(bmode_data.shape[0])):
        if np.unique(bmode_mask[i,:,:,]).shape[0]==1:
            print('Vacio '+str(i))
            vacios.append(i)
    bmode_mask = np.delete(bmode_mask,vacios,axis=0)
    bmode_data = np.delete(bmode_data,vacios,axis=0)
    #cuadradar mascaras vacias, datos y labels por indice
    copiaLabels3 = np.delete(copiaLabels3,vacios)
    copiaLabels0 = np.delete(copiaLabels0,vacios)
    copiaLabels1 = np.delete(copiaLabels1,vacios)

    new_data = np.zeros([620,2400,440])
    new_mask = np.zeros([620,2400,440])
    for frame in range(0,bmode_data.shapel[0]):
        print(bmode_data[frame].shape)
        imagenin = bmode_data[frame]
        maskin = bmode_mask[frame]
        HEIGHT = int(imagenin.shape[0]/2*3)
        WIDTH = int(imagenin.shape[1]/2)
        imagenout = transform.resize(imagenin, output_shape=(HEIGHT,WIDTH), anti_aliasing = True)
        maskout = transform.resize(maskin, output_shape=(HEIGHT,WIDTH), anti_aliasing = True)
        maskout1 = np.array(maskout)
        maskout1[maskout>=0.5]=1
        maskout1[maskout<0.5]=0
        new_data[frame] = imagenout
        new_mask[frame] = maskout1
    '''j=0
    for jdj in bmode_labels[:,3]:
        if jdj == -1:
          bmode_labels[j][3] = 0
        if jdj == 2:
          bmode_labels[j][3] = 0
        ...
        j=j+1
    j=0 
    bmode_labels[:,3][bmode_labels[:,3]!=1]=0'''  
    #copiaLabels3[copiaLabels3!=3]=0 
    #copiaLabels3[copiaLabels3==3]=1
    bmode_labels_unicos = np.unique(copiaLabels1)
    labelPrueba = np.zeros(len(bmode_labels_unicos))
    j=0
    for mode in bmode_labels_unicos:
        indice = np.where(copiaLabels1==mode)
        labelPrueba[j]=indice[0][0]
        j = j+1
    labelPrueba = labelPrueba.astype(int)
    labelPrueba = copiaLabels3 [labelPrueba]
    labelPrueba = np.array(labelPrueba)
    labelPrueba = labelPrueba.astype(int)    
    skf.get_n_splits(bmode_labels_unicos,labelPrueba)
    j=0
    indiceTotal = []    
    for train_index, test_index in skf.split(bmode_labels_unicos,labelPrueba):
        sumadata=[]
        sumamask=[]
        print('TRAIN:', train_index, 'TEST:', test_index)
        print('Ejecutando cross validation ' +str(j)+ ' de 3')
        train_index1=bmode_labels_unicos[train_index]
        test_index1=bmode_labels_unicos[test_index]
        i=0
        for train in train_index1:
            indice = np.where(copiaLabels1==train)
            for io in indice[0]:
              sumadata.append(new_data[io])
              sumamask.append(new_mask[io])
              indiceTotal.append(io)
        
        ecogeneicidad = pd.DataFrame(copiaLabels3[indiceTotal])
        ecogeneicidad['Paciente_id'] = copiaLabels1[indiceTotal]
        ecogeneicidad['indice'] = copiaLabels0[indiceTotal]      
        locals()["Ytrain_kfold_Clas"+str(j)]=pd.DataFrame(ecogeneicidad)
        locals()["train_kfold"+str(j)]=sumadata
        locals()["trainMask_kfold"+str(j)]=sumamask
        i=0
        sumadata = []
        sumamask = [] 
        indiceTotal = []
        for test in test_index1:
            indice = np.where(copiaLabels1==test)
            for io in indice[0]:
              sumadata.append(new_data[io])
              sumamask.append(new_mask[io])
              indiceTotal.append(io)          
        locals()["test_kfold"+str(j)]=sumadata
        locals()["testMask_kfold"+str(j)]=sumamask
        #locals()["test_set_kfold"+str(j)]=test_index
        ecogeneicidad = pd.DataFrame(copiaLabels3[indiceTotal])
        ecogeneicidad['Paciente_id'] = copiaLabels1[indiceTotal]
        ecogeneicidad['indice'] = copiaLabels0[indiceTotal]
        locals()["Ytest_kfold_Clas"+str(j)] = pd.DataFrame(ecogeneicidad)
        j=j+1 


    for i in range(0,j):
        np.save(path_to_raw + 'train' + str(i)+'_DATA', locals()["train_kfold"+str(i)])
        np.save(path_to_raw + 'train' + str(i)+'_MASK', locals()["trainMask_kfold"+str(i)])
        np.save(path_to_raw + 'train' + str(i)+'_LABELS', locals()["Ytrain_kfold_Clas"+str(i)])
        np.save(path_to_raw + 'test' + str(i)+'_DATA', locals()["test_kfold"+str(i)])
        np.save(path_to_raw + 'test' + str(i)+'_MASK', locals()["testMask_kfold"+str(i)])
        np.save(path_to_raw + 'test' + str(i)+'_LABELS', locals()["Ytest_kfold_Clas"+str(i)])
'''
if prepare_train_test_split == True:
...
if load_existing_dataset:
    ...
    #path_to_raw = "../QUS_48x61_newfeatsQUS_RaulSeq_datasetsmall/"
    path_to_raw = "/home/estudiante/rfvsbmode_Ignacio/rfvsbmode_Ignacio/datosEcogeneicidad/"
    inputnn = "/home/estudiante/rfvsbmode_Ignacio/rfvsbmode_Ignacio/inputnn_PHILIPSBMODEEcogenicidadSinMascara/"

    ########### 1-Perform RF vs B-mode conversion + spectrogram calculation #####################################
    skipRFvsBmodeConversion = False
    if (skipRFvsBmodeConversion == True):
        #Variables
        ...
        #list_labels_modify_functions = [ ["_iqv2p0", stages_rf2D_rftobmode_iq_rev2p0],
        #                                  ["_carrierv2p0", stages_rf2D_carrierfun_rev2p0],
        #                                  ["_anglev2p0", stages_rf2D_angle_rev2p0],
        #                                  ["_psphasev2p0", stages_rf2D_ps_phase_rev2p0]]
        list_labels_modify_functions = [[ "_carrierv3p0", stages_rf2D_carrierfun_rev2p0],
                                        [ "_denv2p0", stages_rf2D_rftobmode_den_rev2p0]]
    ...
    skipGenerate2DPatches = False
    if (skipGenerate2DPatches == False):
        Npatches = 150#1500 #300
        Naxial = 120#30#120#392
        Nlines = 120#30#120 #27
        #Variables
        whichdata = ['test', 'train']
        oli=0
        ...
        for kfold in cross_validation_folds:        #Data evaluation loop
            ...
            for xdata in whichdata:
                ...
                print('Cargando datos para la generación de patches....')
                labframe = np.load( (path_to_raw + xdata + str(kfold) + '_LABELS.npy'), allow_pickle=True) 
                RF_mask = np.load( (path_to_raw + xdata + str(kfold) + '_MASK.npy')) # same size as trainRF_array, contains segmentation masks
                RF_array = np.load( (path_to_raw + xdata + str(kfold) + '_DATA.npy')) # same size as trainRF_array, contains segmentation masks
                print('Datos cargados!!')
                ...
                plt.show()
                '''
                #RF_mask = RF_mask[:,::4,::4] #comprimimos para poder generar mas patches
                #RF_array = RF_array[:,::4,::4]
                ...
                for index in range(Nframes):
                    patientId = int(labframe[index,1])
                    print('Frame: ', str(index) + ' of ' + str(Nframes))
                    largest_connected_region =  find_largest_connected_region(RF_mask[index,:,:])
                    if len(str(largest_connected_region)) == 5:
                        largest_connected_region = find_largest_connected_region(RF_mask[56,:,:])
                        oli = oli+1
                    ...
                    #        largest_connected_region = find_largest_connected_region(RF_mask[56,:,:])
                    #        oli = oli+1
                    #    largest_connected_region = binary_fill_holes(largest_connected_region) # se para
                    #    mask_patch, coverage[index], where_patches = mask_patch_extraction(RF_array[index,:,:], largest_connected_region, Naxial+1, Nlines, Npatches) # mask_patch (393, 27, 15) <> +1 in axial coordinate to have odd number
                    if coverage[index] > 0:
    ...
    # DONE
    # Data extraction 
    skipGenerate2DPatchesFromRFvsBmode = True
    if (skipGenerate2DPatchesFromRFvsBmode == False):
        ...
        #list_labels_modify = ["_iqv2p0", "_carrierv2p0", "_anglev2p0", "_psphasev2p0"]
        #list_labels_modify = ["_carrierv3p0",  "_denv2p0"]
...
if load_existing_dataset == True:
    SystemExit()

# provide number of cores available for use
#print("Available CPU cores: " + str(num_cores))

MODIFIER = '_PHILIPSBMODEEcogenicidadSinMascara'
MODIFIER_STORE = '_PHILIPSBMODE_RF'
path_to_raw = '/home/estudiante/rfvsbmode_Ignacio/rfvsbmode_Ignacio/'
inputnn = path_to_raw+"/inputnn" + MODIFIER + "/"
outputnnmodel = path_to_raw+"/outputnnmodel" + MODIFIER_STORE + "/"
outputnnresults = path_to_raw+"/outputnnresults" + MODIFIER_STORE + "/"
numClasses = 2

# We open model library
# ------------------------------------------------------------------------------------------------------------------------------------
#exec(open("0_main_modellibrary_rfvsbmode_OMAR.py").read())
exec(open("/home/estudiante/rfvsbmode_Ignacio/rfvsbmode_Ignacio/0_main_modellibrary_rfvsbmode_OMAR.py").read())
...
dict_train_settings2DEcofrail = {
    "numEpochs": 10,
    "batchSize": 30,
    "learning_rate": 0.01,
}
dict_train_settings2DEcofrail01 = {
    "numEpochs": 10,
    "batchSize": 30,
    "learning_rate": 0.10,
}
...
#"_carrierv3p0" "_denv2p0"
list_labels_training = [
                        #["array_iqv2p0", "_labels2D", stages_dont_do_anything, model_2D_210411_06, "Run210516_model_2D_210411_06" , dict_train_settings2D],
                       # ["array_carrierv3p0", "_labels2D", stages_dont_do_anything, model_2D_210411_06, "Run210516_model_2D_210411_06" , dict_train_settings2D],
                        #["array_anglev2p0", "_labels2D", stages_dont_do_anything, model_2D_210411_06, "Run210516_model_2D_210411_06" , dict_train_settings2D],
                        #["array_psphasev2p0", "_labels2D", stages_reshape3D_2Dpatch, model_2D_210411_06, "Run210516_model_2D_210411_06" , dict_train_settings2D],
                        #["array_psphasev2p0", "_labels2D", stages_reshape3D_4Dpatch, model_3D_210411_07_optim, "Run210516_model_3D_210411_07bis" , dict_train_settings3D],
                       # ["array", "_labels2D", stages_dont_do_anything, model_2D_210411_06, "Run210425_model_2D_210411_06", dict_train_settings2D],
                       ...
                       # ["array_log", "_labels2D", stages_dont_do_anything, model_2D_210411_06b, "Run210425_model_2D_210411_06bis", dict_train_settings2D],
                        ["array", "_labels2D", stages_dont_do_anything, model_2D_210411_06bEcofrail08, "Run210425_model_2D_210411_06bisEcofrail", dict_train_settings2DEcofrail01],
                       # ["array_phasefun", "_labels2D", stages_dont_do_anything, model_2D_210411_06, "Run210512_model_2D_210411_06", dict_train_settings2D],
                       ...
                       # ["array_pslog", "_labels2D", stages_reshape3D_4Dpatch, model_3D_210411_07_optim, "Run210425_model_3D_210411_07bis", dict_train_settings3D],
                       # ["array_pslog", "_labels2D", stages_reshape3D_2Dpatch, model_2D_210411_06, "Run210425_model_2D_210411_06", dict_train_settings2D],
                       ...
                       # ["array_pslognormden", "_labels2D", stages_reshape3D_2Dpatch, model_2D_210411_06,  "Run210425_model_2D_210411_06", dict_train_settings2D],   
                      ...
                      #  ["array_pslogfirst", "_labels2D", stages_reshape3D_4Dpatch, model_3D_210411_07_optim, "Run210425_model_3D_210411_07bis", dict_train_settings3D],
                       ...
                       # ["array_iq", "_labels1D", stages_reshape2D_1Dpatch, model_1D_210418_08, "Run210425_model_1D_210411_08", dict_train_settings1D],
                       # ["array_env", "_labels1D", stages_reshape2D_1Dpatch, model_1D_210418_08b, "Run210425_model_1D_210411_08b", dict_train_settings1D],
                       # ["array_log", "_labels1D", stages_reshape2D_1Dpatch, model_1D_210418_08b, "Run210425_model_1D_210411_08b", dict_train_settings1D],
                       # ["array_den", "_labels1D", stages_reshape2D_1Dpatch, model_1D_210418_08b, "Run210425_model_1D_210411_08b", dict_train_settings1D],
                 #       ["array_phasefun", "_labels1D", stages_reshape2D_1Dpatch, model_1D_210418_08, "Run210425_model_1D_210411_08", dict_train_settings1D],
                        ]
...
'''
list_labels_training = [
                        ["array", "_labels2D", stages_dont_do_anything, model_2D_210515_06_elu_last2, "Run210515_model_2D_210515_06_elu", dict_train_settings2D],
                        ["array_env", "_labels2D", stages_dont_do_anything, model_2D_210411_06, "Run210515_model_2D_210411_06", dict_train_settings2D],
                        ["array_den", "_labels2D", stages_dont_do_anything, model_2D_210411_06, "Run210515_model_2D_210411_06", dict_train_settings2D],
                        ["array_ps", "_labels2D", stages_reshape3D_4Dpatch, model_3D_210515_07_optim_allrelus, "Run210515_model_3D_210515_07_optim_allrelus", dict_train_settings3D],
                        ["array_pslog", "_labels2D", stages_reshape3D_4Dpatch, model_3D_210515_07_optim_allrelus, "Run210515_model_3D_210515_07_optim_allrelus", dict_train_settings3D],
                        ["array_pslognormden", "_labels2D", stages_reshape3D_4Dpatch, model_3D_210515_07_optim_allrelus, "Run210515_model_3D_210515_07_optim_allrelus", dict_train_settings3D],
                        ["array_ps", "_labels2D", stages_reshape3D_4Dpatch, model_3D_210515_07_optim_maxpools, "Run210515_model_3D_210515_07_optim_maxpools", dict_train_settings3D],
                        ["array_pslog", "_labels2D", stages_reshape3D_4Dpatch, model_3D_210515_07_optim_maxpools, "Run210515_model_3D_210515_07_optim_maxpools", dict_train_settings3D],
                        ["array_pslognormden", "_labels2D", stages_reshape3D_4Dpatch, model_3D_210515_07_optim_maxpools, "Run210515_model_3D_210515_07_optim_maxpools", dict_train_settings3D],
                        ["array_ps", "_labels2D", stages_reshape3D_2Dpatch, model_2D_210515_06_allrelus, "Run210425_model_2D_210411_06", dict_train_settings2D],
                        ["array_pslog", "_labels2D", stages_reshape3D_2Dpatch, model_2D_210515_06_allrelus, "Run210425_model_2D_210411_06", dict_train_settings2D],
                        ["array_pslognormden", "_labels2D", stages_reshape3D_2Dpatch, model_2D_210515_06_allrelus,  "Run210425_model_2D_210411_06", dict_train_settings2D],
                        ]
'''
...
if skipTraining == False:
    ...
    #cross_validation_folds = [2,3]  #, 4
    cross_validation_folds = [0]#[0,1,2,3] #[0,1,2] #[3,4]
    ...
    for x in list_labels_training:
        ...
        for kfold in cross_validation_folds:        #Data evaluation loop
            ...
            #Cargando los datos comprimidos previamente generados
            '''train_data=np.load('/home/estudiante/rfvsbmode_Ignacio/rfvsbmode_Ignacio/datosComprimidos3vsResto-1500patch/patch_train.npy')
            val_data=np.load('/home/estudiante/rfvsbmode_Ignacio/rfvsbmode_Ignacio/datosComprimidos3vsResto-1500patch/patch_val.npy')
            labels_train=np.load('/home/estudiante/rfvsbmode_Ignacio/rfvsbmode_Ignacio/datosComprimidos3vsResto-1500patch/patch_labels_train.npy')
            labels_val=np.load('/home/estudiante/rfvsbmode_Ignacio/rfvsbmode_Ignacio/datosComprimidos3vsResto-1500patch/patch_labels_val.npy')'''
            train_data = modify_fun(train_data)
            val_data = modify_fun(val_data)
            ...
            #From 150 patches to 15 images of 10 patches (1200x120)(300x30)
            '''patch_train = np.zeros([int(train_data.shape[0]/10),300,30,1])
            patch_val = np.zeros([int(val_data.shape[0]/10),300,30,1])
            patch_labels_train = np.zeros([int(train_data.shape[0]/10),4])
            patch_labels_val = np.zeros([int(val_data.shape[0]/10),4])
            for index in range(0,patch_train.shape[0]):
                NPATCHESLONG=10
                patch_train[index:,:, ] = np.concatenate(train_data[(index*NPATCHESLONG): (index+1)*NPATCHESLONG], axis = 0)
                patch_labels_train[index]=labels_train[index*NPATCHESLONG]
            for index in range(0,patch_val.shape[0]):
                NPATCHESLONG=10
                patch_val[index:,:, ] = np.concatenate(val_data[(index*NPATCHESLONG): (index+1)*NPATCHESLONG], axis = 0)
                patch_labels_val[index]=labels_val[index*NPATCHESLONG]
            patch_train = patch_train[:,::2,::2,:]
            patch_val = patch_val[:,::2,::2,:]
            np.save('/home/estudiante/rfvsbmode_Ignacio/rfvsbmode_Ignacio/datosComprimidos01vs23-1500patch/patch_train.npy',patch_train)'''

            ## Here we could include modifier functions for regression or other label classification...
            if (numClasses == 2): 

                labels_train_1hot = one_hot_encode(numClasses, labels_train[:,1])
                labels_val_1hot = one_hot_encode(numClasses, labels_val[:,1])
            ...
            my_optimizer = tf.keras.optimizers.Adadelta( learning_rate=lr_schedule, rho=0.95 , epsilon=1e-07 ) # el que hemos utilizado siempre
            #else:
    ...
    ########################################## TRAINING LOOP ######################################
            ...
            '''ind_list = [i for i in range(labels_train.shape[0])]  #N is number of patches
            shuffle(ind_list)
            train_data = train_data[ind_list, :,:,:]
            labels_train_1hot = labels_train_1hot[ind_list,:]

            ind_list2 = [i for i in range(val_tuple[0].shape[0])]  #N is number of patches
            shuffle(ind_list2)
            val_tuple[0][:,:,:,:] = val_tuple[0][ind_list2, :,:,:]
            val_tuple[1][:,:] = val_tuple[1][ind_list2, :]'''
    ...
    ##### DATA ANALYSIS AND EVALUATION LOOP ##################################################################################################
            ...
            if do_pred_train:
                ...
                print('\n\\\\\\\\\\ CASE LEVEL /////')
                train_real_labels_c, train_test_case_df, train_test_labels_c, train_metrics_c = results_caseEcofrail( pred_train , labels_train ,destination_results, '_train_', threshold ) 
            ...
            if do_pred_val:
                ...
                print('\n\\\\\\\\\\ CASE LEVEL /////')
                val_real_labels_c, val_test_case_df, val_test_labels_c, val_metrics_c = results_caseEcofrail( pred_val , labels_val,destination_results, '_val_', threshold )
    ...
    # -----------------------------------------------------------------------------------------------------------------------------------------------------------
                ...
                # labelling data
                r_df['numClasses'] = numClasses'''#aquiii
                ...
                # save metrics
                r_df.to_pickle( (destination_results + '/results.pkl') )'''#aquii
