...
if prepare_train_test_split == True:
    ...
    np.save(path_to_raw + 'train_' + DATA_NAME, bmode_data_train)
    np.save(path_to_raw + 'test_' + DATA_NAME, bmode_data_test)
    np.save(path_to_raw + 'train_' + LABELS_NAME, bmode_labels_train)
    np.save(path_to_raw + 'test_' + LABELS_NAME, bmode_labels_test)
    np.save(path_to_raw + 'train_' + MASK_NAME, bmode_mask_train)
    np.save(path_to_raw + 'test_' + MASK_NAME, bmode_mask_test)
...
if load_existing_dataset:
    ...
    if (skipGenerate2DPatches == False):
        ...
        for xdata in whichdata:
            ...
            labframe = np.load( (path_to_raw + xdata +  '_' + LABELS_NAME) ) 
            RF_mask = np.load( (path_to_raw + xdata + '_' + MASK_NAME)) # same size as trainRF_array, contains segmentation masks
            RF_array = np.load( (path_to_raw + xdata + '_' + DATA_NAME)) # same size as trainRF_array, contains segmentation masks
            ...
            np.save(inputnn + xdata + 'B_patches_' +   'array.npy', patches2D[0:idwrite,:,:,:])
            np.save(inputnn + xdata + 'B_patches_' +  '_labels2D.npy', labels2D[0:idwrite,:])
            np.save(inputnn + xdata + 'B_patches_' +  '_labels1D.npy', labels1D[0:idwrite1D,:])
            np.save(inputnn + xdata + 'B_patches_' +  '_where_patches_receipt.npy', where_patches_receipt[0:idwrite,:])
...
list_labels_training = [
                      ...
                      #  ["array_den", "_labels1D", stages_reshape2D_1Dpatch, model_1D_210418_08b, "Run210421_model_1D_210411_08b", dict_train_settings1D],
                        ["array", "_labels2D", stages_dont_do_anything, PHILIPS_N35_bmode_model_2D_210411_06b, "Run210424_PHILIPS_N35_bmode_model_2D_210411_06b_0p01LR_30batch", dict_train_settings2Dbis],
...
if skipTraining == False:
    ...
    for x in list_labels_training:
        ...
        train_val_RAM = True
        train_data = np.load(inputnn + "trainB_patches_" + dataset_name + '.npy') # Already prepared for training
        train_data = modify_fun(train_data)  # Perform reshaping if necessary - check memory concerns
        val_data = np.load(inputnn + "testB_patches_" + dataset_name + '.npy')
        ...
        # Get labels
        labels_train = np.load(inputnn + "trainRF_patches_" + labels_name + '.npy')
        labels_val = np.load(inputnn + "testRF_patches_" + labels_name + '.npy')
...
########################################## TRAINING LOOP ######################################
        ...
        if os.path.exists(outputnnmodel + model_label + '__' + dataset_name ):
            shutil.rmtree(outputnnmodel + model_label + '__' + dataset_name )

        os.mkdir( outputnnmodel+ model_label + '__' + dataset_name)
        model.save( (outputnnmodel +model_label  + '__' + dataset_name + '/model') )
        gc.collect()
        if os.path.exists(outputnnresults  + model_label + '__' + dataset_name):
            shutil.rmtree(outputnnresults  + model_label + '__' + dataset_name)
        os.mkdir(outputnnresults  + model_label + '__' + dataset_name)
